ruleset io.picolabs.manage_sensors {
  meta {
    shares __testing, sensors, temperatures, get_all_temperatures, reports
  ...
  rule notify_needed_temperature{
    select when sensor temperature_report_notification
      foreach subscriptions:established("Tx_role", "sensor").klog("mysubscription") setting (s)
        pre{
          tx = s{"Tx"}
        }
        event:send({"eci": tx,
  ...
  rule receive_temperature_report{
    select when sensor receive_temperature_report
    pre{
      name = event:attr("name")
      temperatures = event:attr("temperatures")
    }
    fired{
			ent:reports{["temperatures", name]} := temperatures;
    }
    else{
